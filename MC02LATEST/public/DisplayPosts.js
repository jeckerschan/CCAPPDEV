$(document).ready(function() {
  ...
  // Function to fetch posts from the server based on the current user's profile
  function fetchPostsForCurrentUser() {
    fetch('http://localhost:3000/posts')
    .then(response => response.json())
    .then(posts => {
        // Filter posts based on current user's profile ID
        const filteredPosts = posts.filter(post => post.profileId === currentUserProfileId);
  ...
  function editPost(postId) {
    ...
    .catch(error => {
        ...
        alert('Failed to edit post on the server');
    });
  }
  ...
  // Call fetchPostsForCurrentUser when MainPage.html is loaded
  $(document).ready(function() {
