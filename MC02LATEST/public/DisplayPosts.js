...
function displayPosts() {
    fetch('http://localhost:3000/posts')
        .then(response => response.json())
        .then(posts => {
            // Assuming you have a global posts array
            // Clear the existing posts array
            samplePosts.length = 0;

            // Populate the posts array with the fetched posts
            posts.forEach(post => {
                samplePosts.push(post);
            });

            // Now you can proceed with displaying the posts
